...
public function searchTracks(array $filters = [], $limit = 50, $offset = 0)
{
    ...
    $searchTerms = [];
    foreach ($filters as $key => $value) {
        $searchTerms[] = is_array($value) ? implode(' ', $value) : $value;
    }
    $searchString = trim(implode(' ', $searchTerms));
    $query = [
        'token'       => $this->apiToken,
...
public function listTracks($searchQuery = '', $limit = 100, $offset = 0,$pg=0)
    {
        $pg=$pg-1;
        $baseUrl = "https://ordiio.sourceaudio.com/api/tracks/search";
        $response = Http::get($baseUrl, [
...
public function availableFilters(Request $request)
{
    ...
    try {
        ...
        if ($response->failed()) {
            throw new \Exception("Failed to fetch filters: " . $response->body());
        }
        $data = $response->json();
        if (!isset($data['filters'])) {
            throw new \Exception("No filters found in SourceAudio response");
        }
        $filters = $data['filters'];
        $renamedFilters = [
            'moods'        => $filters['moods'] ?? [],
            'genres'       => $filters['genre'] ?? [],
            'regions'      => $filters['custom_487'] ?? [],
            'areas'        => $filters['custom_3135'] ?? [],
            'useCases'     => $filters['custom_873'] ?? [], 
            'instruments'  => $filters['instruments'] ?? [],
            'tempo'        => $filters['tempo'] ?? [],
        ];
        unset($filters['hasVocals']);
        $formatted = [];
        foreach ($renamedFilters as $key => $items) {
            $options = [];
            foreach ($items as $item) {
                $options[] = [
                    'label' => $item['text'] ?? ucfirst($key),
            ...
            $formatted[] = [
                'filter_key' => $key,
                'filter_label' => ucfirst(str_replace('_', ' ', $key)),
                'options' => $options,
            ];
        }
        return $formatted;
    } catch (\Exception $e) {
        throw new \Exception("SourceAudio API Error: " . $e->getMessage());
    }
}
