...
<head>
  <meta charset="UTF-8" />
  <title>Croose | Pay for Your Appointment</title>
  ...
  <style>
    * {
    ...
    .container {
      ...
      animation: fadeInCard 1.4s ease-out;
    }
...
<body>
  ...
  <div>
    ...
    <div class="container">
      ...
      <div class="info">
        <strong>Service:</strong> {{ $appointment->service->name ?? 'N/A' }} <br />
        <strong>Amount:</strong> {{ $appointment->amount }} {{ strtoupper($space->currency ?? '') }}
      </div>

      {{-- Stripe (USD) --}}
      <!-- <a href="javascript:void(0)" class="btn btn-stripe" onclick="redirectWithLoader('{{ route('payment.stripe.appointment', ['uuid' => $uuid]) }}')">
        <img src="https://cdn-icons-png.flaticon.com/512/349/349221.png" alt="Stripe Icon" />
        Pay with Stripe (USD)
      </a> -->

      {{-- Paystack (GHS) --}}
      <a href="javascript:void(0)" class="btn btn-paystack" onclick="redirectWithLoader('{{ route('payment.paystack.appointment', ['uuid' => $uuid]) }}')">
