...
<head>
  <meta charset="UTF-8" />
  <title>Croose | Pay for Your Order</title>
  ...
  <div>
    ...
    <div class="container">
      ...
      <div class="info">
        ...
        <strong>Amount:</strong> {{ $order->order_amount }} {{ strtoupper($space->currency ?? '') }}
      </div>

      {{-- Stripe (USD) --}}
      <!-- <a href="javascript:void(0)" class="btn btn-stripe" onclick="redirectWithLoader('{{ route('payment.stripe.order', ['uuid' => $uuid]) }}')">
        <img src="https://cdn-icons-png.flaticon.com/512/349/349221.png" alt="Stripe Icon" />
        Pay with Stripe (USD)
      </a> -->

      {{-- Paystack (GHS) --}}
      <a href="javascript:void(0)" class="btn btn-paystack" onclick="redirectWithLoader('{{ route('payment.paystack.order', ['uuid' => $uuid]) }}')">
